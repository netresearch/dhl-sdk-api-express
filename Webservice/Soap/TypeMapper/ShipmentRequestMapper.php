...
class ShipmentRequestMapper
{
     ...
     */
    public function map(ShipmentRequestInterface $request): SoapShipmentRequest
    {
        ...
        if ((count($shipperStreetLines) > 1) && ('' !== $shipperStreetLines[1])) {
            $requestedShipment->getShip()->getShipper()->getAddress()->setStreetLines2($shipperStreetLines[1]);
        }
        if ((count($shipperStreetLines) > 2) && ('' !== $shipperStreetLines[2])) {
        ...
        if ((count($recipientStreetLines) > 1) && ('' !== $recipientStreetLines[1])) {
            $requestedShipment->getShip()->getRecipient()->getAddress()->setStreetLines2($recipientStreetLines[1]);
        }
        if ((count($recipientStreetLines) > 2) && ('' !== $recipientStreetLines[2])) {
